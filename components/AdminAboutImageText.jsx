...
export default function AdminAboutImageText({ sectionId, title, subsectionNumber, expanded, onToggle }) {
      ...
      {expanded && (
        <div className="subsection-body">
          <div className="form-grid">
            ...
            <div className="input-group form-full-width">
              <label className="input-label">Paragraphs (Separate by newlines)</label>
              <textarea className="text-input textarea-input" style={{minHeight: "150px"}} value={data.text} onChange={e => setData({...data, text: e.target.value})} />
            </div>
            
            <div className="input-group form-full-width">
              <label className="input-label">Image (Cloudinary)</label>
              <div className="media-upload-zone" onClick={() => fileInputRef.current?.click()}>
                <span className="upload-icon">🖼️</span>
                <span className="upload-text">{file ? file.name : "Upload new image"}</span>
