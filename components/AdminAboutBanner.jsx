...
export default function AdminAboutBanner({ expanded, onToggle }) {
  ...
  const fetchData = async () => {
    ...
    if (res.ok) {
      ...
      const item = items.find(i => i.title === "about_banner");
      if (item) {
        setData({
          id: item.id,
          title: item.text || "", 
  ...
  const handleSave = async () => {
    ...
    form.append("title", "about_banner");
    ...
    try {
      ...
      if (res.ok) {
        await fetchData();
        setFile(null);
        alert("About Banner updated successfully!");
      } else {
