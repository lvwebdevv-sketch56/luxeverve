...
export default function AdminSection3({ expanded, onToggle }) {
  ...
  const fetchSection = async () => {
    ...
    if (res.ok) {
      ...
      setAllMedia(items.filter(i => i.title !== "home_section3"));
    }
  };
  ...
  const handleSave = async () => {
    ...
    form.append("title", "home_section3");
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Failed to update Section 3: ${err.error}`);
      }
    } catch (e) {
    ...
    <div className={`subsection-card ${expanded ? "open" : ""}`}>
      <div className="subsection-header" onClick={onToggle}>
        <div className="subsection-title-box">
          ...
          <span className="subsection-title">Section 3: Text Left, Video Right</span>
        </div>
